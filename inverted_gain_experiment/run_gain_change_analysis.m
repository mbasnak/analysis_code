...
for fly = 1:length(data_dirs)
    ...
    for file = 1:length(fly_files)
        if contains(fly_files(file).name,['sid_',num2str(sid),'_']) & contains(fly_files(file).name,'continuous_analysis')
            %load the data
            ...
            save([path,'\gain_change_data.mat'],'modelTable','modelTableNG','type_of_fly','ratio_of_offset_var','movementData');
