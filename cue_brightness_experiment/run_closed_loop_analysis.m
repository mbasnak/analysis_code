...
for fly = 1:length(data_dirs)
    ...
    for file = 1:length(fly_files)
        if (contains(fly_files(file).name,['sid_',num2str(sid),'_']) & contains(fly_files(file).name,'continuous'))
            ...
            %save figure
            saveas(gcf,[path,'\continuous_plots\closed_loop_heatmap_and_offset.png']);
            ...
            %save figure
            saveas(gcf,[path,'\continuous_plots\closed_loop_offset_variability.png']);
            ...
            subplot(2,1,1)
            for block = 1:length(blockLimits)
                %Compute offset precision
                good_fit_block = continuous_data.adj_rs(blockLimits{block}(1):blockLimits{block}(2));
                moving_block = continuous_data.total_mvt_ds(blockLimits{block}(1):blockLimits{block}(2));
                offset_block = deg2rad(offset(blockLimits{block}(1):blockLimits{block}(2)));
                offset_precision(block) = circ_r(offset_block(moving_block > 25),[],[],1);
            ...
            %save figure
            saveas(gcf,[path,'\continuous_plots\closed_loop_offset_precision.png']);
            ...
            for block = 1:length(blockLimits)
                ...
                moving{block} = continuous_data.total_mvt_ds(:,blockLimits{block}(1):blockLimits{block}(2));
            end
            ...
            %4) Save figure
            saveas(gcf,[path,'\continuous_plots\closed_loop_bump_mag_in_time.png']);
            ...
            mean_bump_mag = [];
            for block = 1:length(blockLimits)
                mean_bump_mag(block) = mean(bump_mag{block}((adj_rs{block}>=0.5)& (moving{block}>=25))); %select only datapoints with good gof
            ...
            %save figure
            saveas(gcf,[path,'\continuous_plots\closed_loop_mean_bump_mag.png']);
            ...
            subplot(2,1,1)
            for block = 1:length(blockLimits)
                mean_bump_width(block) = nanmean(bump_width{block}((adj_rs{block}>=0.5) & (moving{block}>=25)));
            ...
            %Save figure
            saveas(gcf,[path,'\continuous_plots\closed_loop_mean_bump_width.png']);
            ...
            %save figure
            saveas(gcf,[path,'\continuous_plots\closed_loop_bump_mag_vs_mvt_per_contrast.png']);
            ...
            %save figure
            saveas(gcf,[path,'\continuous_plots\closed_loop_bump_width_vs_mvt_per_contrast.png']);
            ...
            %save figure
            saveas(gcf,[path,'\continuous_plots\closed_loop_heading_variability.png']);
            ...
            %save figure
            saveas(gcf,[path,'\continuous_plots\closed_loop_heading_precision.png']);         
