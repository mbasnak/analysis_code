...
ggplot(bump_pars_evo_data_bar_trial, aes(time, bump_mag, group = fly, color = factor(fly)))+ 
  ...
  labs(x = 'Time (sec)', y='Bump amplitude (\u0394F/F)', color = '',fill = '')

ggsave(path = "C:/Users/Melanie/Dropbox (HMS)/Manuscript-Basnak/Figures/SupFig2", file="bump_mag_evo_ind_flies.svg",device = 'svg', width=8, height=6)
