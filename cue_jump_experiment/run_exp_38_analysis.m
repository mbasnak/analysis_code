function run_exp_38_analysis()

clear all; close all;

foldernames = dir('Z:\Wilson Lab\Mel\Experiments\Uncertainty\Exp38\data\third_version');
        ...
        for file = 1:length(fileNames)
            if contains(fileNames(file).name,'continuous_analysis')
                ...
                for fly = 1:length(flyNames)
                    if strcmp(flyNames(fly).name,path(68:end-9))
                        fly_ID = fly;
                    end
                end
                ...
                if configuration == 1
                    ...
                    initial_bar_correct_bump_mag = continuous_data.bump_magnitude(initial_bar_period) < 5;
                    ...
                    initial_bar_bm = initial_bar_bm(initial_bar_moving & initial_bar_good_fit & initial_bar_correct_bump_mag);
                    initial_bar_bw = continuous_data.bump_width(initial_bar_period);
                    initial_bar_bw = initial_bar_bw(initial_bar_moving & initial_bar_good_fit & initial_bar_correct_bump_mag);
                    ...
                    initial_wind_correct_bump_mag = continuous_data.bump_magnitude(initial_wind_period) < 5;
                    ...
                    initial_wind_bm = initial_wind_bm(initial_wind_moving & initial_wind_good_fit & initial_wind_correct_bump_mag);
                    initial_wind_bw = continuous_data.bump_width(initial_wind_period);
                    initial_wind_bw = initial_wind_bw(initial_wind_moving & initial_wind_good_fit & initial_wind_correct_bump_mag);
                ...
                for jump = 1:length(real_bar_jump_frames)
                    ...
                    pre_bar_jump_good_fit = continuous_data.adj_rs(pre_bar_jump_period) > 0.45;
                    ...
                    post_bar_jump_good_fit = continuous_data.adj_rs(post_bar_jump_period) > 0.45;
                    ...
                    long_pre_bar_jump_good_fit = continuous_data.adj_rs(long_pre_bar_jump_period) > 0.45;
                    ...
                    long_post_bar_jump_good_fit = continuous_data.adj_rs(long_post_bar_jump_period) > 0.45;
                ...
                for jump = 1:length(real_wind_jump_frames)
                    ...
                    pre_wind_jump_good_fit = continuous_data.adj_rs(pre_wind_jump_period) > 0.45;
                    ...
                    post_wind_jump_good_fit = continuous_data.adj_rs(post_wind_jump_period) > 0.45;
                    ...
                    long_pre_wind_jump_good_fit = continuous_data.adj_rs(long_pre_wind_jump_period) > 0.45;
                    ...
                    long_post_wind_jump_good_fit = continuous_data.adj_rs(long_post_wind_jump_period) > 0.45;
                ...
                for jump = 1:length(real_bar_jump_frames)
                    ...
                    long_bar_offset_mean_around_bar_jump(jump,2) = circ_mean(bar_offset(real_bar_jump_frames(jump)+30*sec_to_frames:real_bar_jump_frames(jump)+150*sec_to_frames));
                    ...
                    long_wind_offset_mean_around_bar_jump(jump,2) = circ_mean(wind_offset(real_bar_jump_frames(jump)+30*sec_to_frames:real_bar_jump_frames(jump)+150*sec_to_frames),[],2);
                    ...
                    long_bar_heading_offset_mean_around_bar_jump(jump,2) = circ_mean(bar_heading_offset(real_bar_jump_frames(jump)+30*sec_to_frames:real_bar_jump_frames(jump)+150*sec_to_frames));
                    ...
                    long_wind_heading_offset_mean_around_bar_jump(jump,2) = circ_mean(wind_heading_offset(real_bar_jump_frames(jump)+30*sec_to_frames:real_bar_jump_frames(jump)+150*sec_to_frames),[],2);
                end
                ...
                for jump = 1:length(real_wind_jump_frames)
                    ...
                    long_bar_offset_mean_around_wind_jump(jump,2) = circ_mean(bar_offset(real_wind_jump_frames(jump)+30*sec_to_frames:real_wind_jump_frames(jump)+150*sec_to_frames));
                    ...
                    long_wind_offset_mean_around_wind_jump(jump,2) = circ_mean(wind_offset(real_wind_jump_frames(jump)+30*sec_to_frames:real_wind_jump_frames(jump)+150*sec_to_frames),[],2);
                    ...
                    long_bar_heading_offset_mean_around_wind_jump(jump,2) = circ_mean(bar_heading_offset(real_wind_jump_frames(jump)+30*sec_to_frames:real_wind_jump_frames(jump)+150*sec_to_frames));
                    ...
                    long_wind_heading_offset_mean_around_wind_jump(jump,2) = circ_mean(wind_heading_offset(real_wind_jump_frames(jump)+30*sec_to_frames:real_wind_jump_frames(jump)+150*sec_to_frames),[],2);
                end
                ...
                %behavior shifts with respect to the cue that doesn't jump
                %Bar moves
                mean_stay_cue_heading_diff_bj = abs(circ_dist(long_wind_heading_offset_mean_around_bar_jump(:,2), long_wind_heading_offset_mean_around_bar_jump(:,1)));
                %Wind moves
                mean_stay_cue_heading_diff_wj = abs(circ_dist(long_bar_heading_offset_mean_around_wind_jump(:,2), long_bar_heading_offset_mean_around_wind_jump(:,1)));
                %compute stickiness index
                mean_stay_cue_heading_diff = [mean_stay_cue_heading_diff_bj;mean_stay_cue_heading_diff_wj];
...
%                 plot(all_bump_mag,'-o')
                ...
                close all; clearvars -except foldernames folder fileNames
