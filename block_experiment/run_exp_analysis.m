%Code to automatically run all experiment analysis

function run_exp_analysis(reliability)

if reliability == 1
    foldernames = dir('Z:\Wilson Lab\Mel\Experiments\Uncertainty\Exp35\data\high_reliability');
else
    foldernames = dir('Z:\Wilson Lab\Mel\Experiments\Uncertainty\Exp35\data\low_reliability');
end
...
for folder = 1:length(foldernames)
    if contains(foldernames(folder).name,'60D05')
        ...
        pre_panels_heading_thresh = pre_panels_heading(continuous_data.total_mvt_ds > 25);
        ...
        pre_panels_offset_above_thresh = deg2rad(offset(continuous_data.total_mvt_ds > 25));
        ...
        pre_wind_heading_thresh = pre_wind_heading(continuous_data.total_mvt_ds > 25);
        ...
        combined_heading_thresh = combined_heading(continuous_data.total_mvt_ds > 25);
        ...
        %store offset for later
        combined_offset_above_thresh = deg2rad(offset(continuous_data.total_mvt_ds>25));
        ...
        post_panels_heading_thresh = post_panels_heading(continuous_data.total_mvt_ds>25);
        ...
        offset = wrapTo180(rad2deg(circ_dist(continuous_data.bump_pos',-continuous_data.heading)));
        %store offset for later
        post_panels_offset_above_thresh = deg2rad(offset(continuous_data.total_mvt_ds>25));
        ...
        post_wind_heading_thresh = post_wind_heading(continuous_data.total_mvt_ds > 25);
        ...
        offset = wrapTo180(rad2deg(circ_dist(continuous_data.bump_pos',-continuous_data.heading)));
        %store offset for later
        post_wind_offset_above_thresh = deg2rad(offset(continuous_data.total_mvt_ds > 25));
        ...
        close all; clearvars -except foldernames folder reliability
